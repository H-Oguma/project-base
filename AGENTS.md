...
2. **AIの担当範囲とワークフロー**:
   ...
   3. **新規ブランチで作業**: コードの実装やテストコードの作成などを新しいブランチ上で行う。また、対応内容に応じて設計書やドキュメントの整備も併せて実施すること。
   4. **対応内容のレビュー**: Pull Requestを作成する前に、必ずコードレビュー用のサブエージェント（`reviewer`）を呼び出して品質や妥当性をレビューさせること。その際、ドキュメントの対応要否（更新漏れがないか等）についても含めてレビューさせ、指摘事項がある場合は修正する。
