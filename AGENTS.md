...
2. **AIの担当範囲とワークフロー**:
   - AIの担当範囲は「コードの作成、テスト、サブエージェントによる事前レビュー、コミット、およびPull Requestの作成」までとします。
   - **事前レビューの必須化**: Pull Requestを作成する前に、必ずコードレビュー用のサブエージェント（`reviewer`）を呼び出して品質や妥当性をレビューさせてください。指摘事項がある場合は修正してからPRを作成してください。
   - **マージ権限**: AIは決してPRを直接マージしないでください。マージは必ず人間（ユーザー）のレビューとApproveを経て行われます。
3. **詳細ルールの参照**: 
